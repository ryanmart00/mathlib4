...
`BanachCat 𝕜` is Triebel's category ℭ₁ (*Interpolation Theory, Function Spaces, Differential
...
structure BanachCat extends ModuleCat.{v} 𝕜 where
...
noncomputable instance : CoeSort (BanachCat.{v} 𝕜) (Type v) := ⟨fun M ↦ M.toModuleCat⟩

attribute [instance] normedAddCommGroup normedSpace completeSpace

/-- Make an object in `BanachCat 𝕜` from an unbundled Banach space. -/
abbrev of (M : Type v) [NormedAddCommGroup M] [NormedSpace 𝕜 M] [CompleteSpace M] :
    BanachCat.{v} 𝕜 :=
  ⟨ModuleCat.of 𝕜 M⟩
...
variable {𝕜} in
/-- Homs in `BanachCat` as one field structures over `ContinuousLinearMap`. -/
structure Hom (X Y : BanachCat.{v} 𝕜) where
  private ofHom' ::
...
instance : Category (BanachCat.{v} 𝕜) where
  Hom := Hom
...
set_option backward.privateInPublic true in
set_option backward.privateInPublic.warn false in
instance : ConcreteCategory (BanachCat.{v} 𝕜) (· →L[𝕜] ·) where
  hom := Hom.hom'
  ofHom := Hom.ofHom'

variable {𝕜} in
/-- Cast a hom in `BanachCat` into a continuous linear map. -/
abbrev Hom.hom {X Y : BanachCat 𝕜} (f : X.Hom Y) : X →L[𝕜] Y :=
  ConcreteCategory.hom (C := BanachCat 𝕜) f

variable {𝕜} in
/-- Construct a hom in `BanachCat` from a continuous linear map. -/
...
@[simp] lemma hom_comp {X Y Z : BanachCat 𝕜} (f : X ⟶ Y) (g : Y ⟶ Z) :
    (f ≫ g).hom = g.hom.comp f.hom := rfl

@[simp] lemma hom_id (X : BanachCat 𝕜) : hom (𝟙 X) = .id _ _ := rfl
